...
void CusbCamConsoleDlg::Disp(LPVOID lpParam)
{
	...
	cv::Mat frame(g_iHeight, g_iWidth, CV_8UC1,pDataBuffer);
	//cv::Mat frame1(g_iHeight, g_iWidth, CV_8UC1, pDataBuffer+(g_iHeight/2)*g_iWidth);
	...
	{
		f_snap = 0;
		int rst=cv::imwrite("Q:\\github\\CqUsbCam_Windows\\examples\\gui\\MT9P031\\usbCamConsole\\Debug\\snap.jpg", frame);
		cout << rst << endl;
	}
	cv::waitKey(1);
}
...
BEGIN_MESSAGE_MAP(CusbCamConsoleDlg, CDialogEx)
	...
	ON_WM_DEVICECHANGE()
	ON_WM_CLOSE()
	ON_BN_CLICKED(IDC_BUTTON_SNAP, &CusbCamConsoleDlg::OnBnClickedButtonSnap)
END_MESSAGE_MAP()
...
void CusbCamConsoleDlg::OnBnClickedRadioResolu12801024()
{
	...
	if (m_bIsCamSelected)
	{
		m_sensorInUse->SetResolution(1);
		g_byteResolutionType = RESOLU_752_480;
		//g_iWidth = 2592 ;
		//g_iHeight = 1944;
	}

	SetDlgItemText(IDC_STATIC_STATUS, L"分辨率2592*1944");
...
void CusbCamConsoleDlg::OnBnClickedButtonSnap()
{
	// TODO: 在此添加控件通知处理程序代码
	f_snap = 1;
}
